...
void update_eeporm_store() {
	for (int i = 0; i < 11; i++)
	{
		calibration_values.calibration_values_min_max[i] = rc_channels.calibration_values[i];
	}

	HAL_I2C_Mem_Write(&hi2c1, EEPROM_ADDRESS, 0, 0xFF,calibration_values.calib_raw_data, 44, 10);
